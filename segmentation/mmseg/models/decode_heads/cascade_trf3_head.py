...
class TransFusion4(nn.Module):
    ...
    def __init__(self, in_chans1, in_chans2, d_model=128, nhead=8, dim_feedforward=512, patch_size1=(12, 12),
                 patch_size2=(12, 12), dropout=0.1, activation='relu', normalize_before=False,
                 norm_cfg=dict(type='BN', requires_grad=True), overlap=False):
...
class Cascade_TRFHead(nn.Module):
    """
    ...
    def __init__(self, in_channels=[], channels=512, d_models=[], n_head=8,
                 ...
                 cascade_num=2,
                 **kwargs):
        super(Cascade_TRFHead, self).__init__(**kwargs)
    ...
    def forward(self, inputs):
        ...
        y1 = self.trf1s[0](inputs[3], inputs[2])
        y2 = self.trf2s[0](y1, inputs[1])
        y3 = self.trf3s[0](y2, inputs[0])
        for i in range(1, self.cascade_num):
            y1 = self.trf1s[i](inputs[3], y1)
            y2 = self.trf2s[i](y1, y2)
            y3 = self.trf3s[i](y2, y3)
